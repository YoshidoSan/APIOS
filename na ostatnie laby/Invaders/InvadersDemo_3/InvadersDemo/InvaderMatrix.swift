...
// Organizacja strzelania
	...
	func remove(_ scene: SKScene,_ invader: Invader){
		...
		// gdy nie ma strzelających, to znaczy, że nie ma już napastników
		if invadersWhoCanFire.isEmpty {
			let nextLevel = NextLevel(size: scene.size)
			nextLevel.scaleMode = scene.scaleMode
			let transitionType = SKTransition.flipHorizontal(withDuration: 1)
			scene.view?.presentScene(nextLevel,transition: transitionType)
		}
	}
